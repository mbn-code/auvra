...
export default function BrandArchivePage({ params }: { params: Promise<{ brand: string }> }) {
  ...
  useEffect(() => {
    const checkMembership = async () => {
      // Use getUser() for secure real-time verification
  ...
  useEffect(() => {
    ...
    if (activeCondition !== "All") result = result.filter(item => item.condition === activeCondition);
    if (activeSize !== "All") result = result.filter(item => item.size === activeSize);

    if (sortBy === "price-asc") result.sort((a, b) => a.listing_price - b.listing_price);
    else if (sortBy === "price-desc") result.sort((a, b) => b.listing_price - a.listing_price);
    else if (sortBy === "savings") result.sort((a, b) => (b.listing_price - (b.member_price || b.listing_price * 0.9)) - (a.listing_price - (a.member_price || a.listing_price * 0.9)));
    else result.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());

    setFilteredItems(result);
  }, [activeCategory, activeCondition, activeSize, sortBy, items, searchQuery]);
  ...
  return (
    <div className="min-h-screen bg-[#fafafa]">
      ...
      <section className="pt-40 pb-20 px-6 border-b border-zinc-100">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-end gap-12">
          <div className="max-w-2xl">
            <Link href="/archive" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-black mb-12 transition-colors">
              <ArrowLeft size={14} />
              All Brands
            </Link>
            ...
            <h1 className="text-6xl md:text-8xl font-black text-zinc-900 mb-8 tracking-tighter leading-[0.95] uppercase italic">
              {decodedBrand}.
            </h1>
          </div>
      ...
      <section className="sticky top-20 z-40 bg-white/80 backdrop-blur-md border-b border-zinc-50 px-6 py-4">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-6">
          <div className="flex items-center gap-4 overflow-x-auto scrollbar-hide flex-1">
            <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all ${showFilters ? "bg-black text-white border-black" : "bg-white text-zinc-900 border-zinc-200"}`}>
              <Filter size={12} />
            ...
            {items.length > 15 && (
              <div className="relative flex-1 max-w-xs hidden md:block">
                ...
                <input type="text" placeholder="Search Archive..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-zinc-50 border border-zinc-100 pl-10 pr-4 py-2.5 rounded-full text-[10px] font-bold uppercase tracking-widest outline-none focus:border-black transition-all" />
              </div>
            )}
          </div>
          <div className="flex items-center gap-4">
            <div className="relative group">
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="appearance-none bg-zinc-50 border border-zinc-100 px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest pr-10 cursor-pointer outline-none hover:border-zinc-300 transition-all">
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        {loading ? (
          <div className="py-32 text-center text-zinc-500 font-bold uppercase tracking-[0.3em] text-[10px]">Syncing Archive...</div>
        ) : filteredItems.length === 0 ? (
          <div className="py-32 text-center">
             <p className="text-zinc-300 font-black text-2xl tracking-tighter mb-4 italic uppercase">"Node search returned null."</p>
          </div>
        ) : (
                  ...
                  <div className={`aspect-[4/5] bg-white rounded-[2.5rem] overflow-hidden mb-8 border border-zinc-100 transition-all duration-700 relative ${isSold ? 'grayscale' : 'group-hover:shadow-2xl group-hover:shadow-zinc-100'} ${isLocked ? 'bg-zinc-950' : ''}`}>
                    ...
                    {isLocked && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
                        ...
                        <span className="text-[8px] font-black text-white uppercase tracking-[0.4em] leading-tight">Society Access Required</span>
                      </div>
                    )}
                    {!isSold && !isLocked && item.images.length > 1 && (
                      <Image src={item.images[1]} unoptimized fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 25vw" className="object-cover opacity-0 group-hover:opacity-100 transition-all duration-1000 scale-110 group-hover:scale-100" alt={`${item.title} alternate`} />
                    )}
                    {isSold && (
